package com.snow.morejobs.gui.architect;
...
public class ArchitectShopContainer extends Container {

    public static ContainerType<ArchitectShopContainer> TYPE;
    private final PlayerEntity player;

    public ArchitectShopContainer(int id, PlayerEntity player) {
        super(TYPE, id);
        this.player = player;
    }
    ...
    public void handleClick(int slotId, boolean rightClick) {
        ...
        ArchitectShopData.ShopItem item = items.get(slotId);
        int amount = rightClick ? 16 : 1;
        int cost = (int) Math.ceil(item.price * amount);

        ServerPlayerEntity serverPlayer = (ServerPlayerEntity) player;
        Item currency = ArchitectShopData.CURRENCY;
